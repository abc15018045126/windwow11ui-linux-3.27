...
function startVlessClientBridge() {
    ...
    ipcMain.on('websocket-open', (event, { connectionId }) => {
        ...
        if (socketState && socketState.stage === 2) {
            ...
            if (vlessHeader) {
                mainWindow.webContents.send('send-websocket-data', { connectionId, data: vlessHeader });
    ...
    const server = net.createServer(socket => {
        ...
        socket.on('data', (data) => {
            ...
            if (socketState.stage === 1) {
                ...
                if (ver !== 5 || cmd !== 1) { socket.destroy(); return; }
...
app.commandLine.appendSwitch('host-resolver-rules', `MAP ${config.serverAddress} ${config.realServerIp}`);
