...
ipcRenderer.on('create-websocket', (event, { connectionId, config }) => {
    ...
    ws.onerror = (error) => {
        ...
        ipcRenderer.send('websocket-error', { connectionId, message: 'A WebSocket error occurred.' });
    };

    ws.onclose = (event) => {
        ipcRenderer.send('websocket-close', { connectionId, code: event.code, reason: event.reason });
