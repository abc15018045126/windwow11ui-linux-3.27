...
const StartMenu: React.FC<StartMenuProps> = ({ onOpenApp, onClose }) => {
  ...
  const pinnedApps = useMemo(() => apps.filter(app => app.isPinned).slice(0, 12), [apps]);
  const recommendedApps = useMemo(() => apps.filter(app => !app.isPinned).slice(0, 6), [apps]);
    ...
    >
      <div className="mb-6 flex-shrink-0">
        <div className="relative">
          ...
          />
          <Icon iconName="search" className="absolute left-3 top-1/2 transform -translate-y-1/2 w-5 h-5 text-zinc-400" />
        </div>
      </div>
